...
describe('Payment Controller - Unit Tests', () => {
  ...
  describe('POST /payments/upload - uploadPayment', () => {
    ...
    it('should return 400 when order_id does not exist', async () => {
      // Mock: order not found (via HTTP 404 from order-service)
  ...
  describe('PUT /payments/:id/verify - verifyPayment', () => {
    it('should return 200 and set verified_at when status is "terverifikasi"', async () => {
      ...
      // Verify HTTP call to order-service
      expect(axios.put).toHaveBeenCalledWith(
        expect.stringContaining('/orders/10/status'),
        { status: 'diproses' }
      );
    });
    ...
    it('should return 200 when status is "ditolak"', async () => {
      ...
      // Should NOT call order-service when rejected
  ...
  describe('GET /payments - getAllPayments', () => {
    it('should return 200 with all payments (no orders JOIN)', async () => {
      const mockPayments = [
        { id: 1, order_id: 1, user_id: 1, status_verifikasi: 'menunggu_verifikasi' },
        { id: 2, order_id: 2, user_id: 2, status_verifikasi: 'terverifikasi' }
      ];
